...
This mechanism is built into "Zoie Realtime Search":http://javasoze.github.com/zoie and Indexing System used to filter out duplicate documents between in-memory indexes and the disk index. (Since they are independent indexes, only consistent id to filter from is the UID)
